...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    if([captureOutput isKindOfClass:[AVCaptureAudioDataOutput class]]){
        ...
        if(shouldSetupContext){
            shouldSetupContext = NO;
            dispatch_sync(dispatch_get_main_queue(), ^{
                CVPixelBufferRef pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
                CGSize resolution = CVImageBufferGetDisplaySize(pixelBuffer);
//                resolution = CGSizeMake(resolution.height, resolution.width);
                [openglView initializeRenderer:resolution];
            });
            
        }
        [openglView updateCameraData:sampleBuffer];
        //             [glk_maskView cameraSampleBuffer:sampleBuffer];
    }
